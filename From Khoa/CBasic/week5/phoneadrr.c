...
int main(void)
{
   ...
   int reval = SUCCESS;
   printf("Read from 3rd data to 6th data \n");
   if ((fp = fopen("phonebook.dat","r+b")) == NULL){
	  ...
	  // Memory allocation
    phonearr = (phoneaddress *)malloc(4 * sizeof(phoneaddress));
    if (phonearr == NULL)
    {
    ...
    irc = fread(phonearr, sizeof(phoneaddress), 4, fp);
    for (i=0; i<4; i++){
   ...
   irc = fwrite(phonearr, sizeof(phoneaddress), 4, fp);
